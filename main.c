...
int main()
{
	...
	while(iterations < numIts)
	{
		...
		for(i = 0; i < MAPSIZE; i++)
			for(j = 0; j < MAPSIZE; j++)
			{
				//if(m[i][j].h <= 3)						//////////  ---> talvez mudar isso <---  //////////
					m[i][j].cor = BRANCO;
			}
		...
		{
			atual = remove_fila(&fila); // posição da fila a ser trabalhada
			...
			for(i = -1; i <= 1; i++)
				for(j = -1; j <= 1; j++)
				{
					...
					if( !(i == 0 && j == 0)															// posição é adjacente
						...
						&& m[temp.x][temp.y].h < m[atual.x][atual.y].h)								// e adjacente é menor que tile da fila
					{
						...
						else  // ou diminui altura em relação ao maior em volta do temp (não necessariamente o tile original da fila)
						{
							...
							for(k = -1; k <= 1; k++)
								for(l = -1; l <= 1; l++)
									if( !(k == 0 && l == 0)
									  	&& (temp.x + l >= 0 && temp.y + k >= 0 && temp.x + l < MAPSIZE && temp.y + k < MAPSIZE)
							...
							if(m[temp.x][temp.y].h == 0)
								continue;
						}

						insere_fila(&fila, temp);	// coloca tile alterado na fila
					}
